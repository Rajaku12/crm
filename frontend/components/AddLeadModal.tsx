...
import { Lead, LeadStatus, LeadTag, PropertyStatus } from '../types';
...
export const AddLeadModal: React.FC<AddLeadModalProps> = ({ isOpen, onClose }) => {
    const { addLead, agents, properties } = useAppContext();
    const initialState = {
        name: '',
        email: '',
        phone: '',
                            ...
                            <div className="mt-2 grid grid-cols-2 md:grid-cols-3 gap-2">
                                {PRODUCT_OPTIONS.map(product => (
                                    <label key={product} className="flex items-center space-x-2">
                                        <input type="checkbox" value={product} onChange={e => handleCheckboxChange(e, 'products')} className="rounded border-gray-300 text-primary-600 shadow-sm focus:border-primary-300 focus:ring focus:ring-offset-0 focus:ring-primary-200 focus:ring-opacity-50" />
                            ...
                            <div className="mt-2 grid grid-cols-2 md:grid-cols-3 gap-2">
                                {SERVICE_OPTIONS.map(service => (
                                    <label key={service} className="flex items-center space-x-2">
                                        <input type="checkbox" value={service} onChange={e => handleCheckboxChange(e, 'services')} className="rounded border-gray-300 text-primary-600 shadow-sm focus:border-primary-300 focus:ring focus:ring-offset-0 focus:ring-primary-200 focus:ring-opacity-50" />
